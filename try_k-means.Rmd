...
```
#### b) View clusters from last iteration
```{r}
df_kmeans[[n_iter]] %>% 
...
```
#### b) Prepate dataset to make polygons
```{r}
df_kmeans %>% 
  ...
  map_dfr( ~ .) -> df_polygon
```
#### c) Visualize clusters as poligons, animate points accesory to clusters (by changing color) and centers of clusters
...
gganimate(gg)
```
#### d) Visualize of Euclidean distance for each points in each iteration
```{r}
